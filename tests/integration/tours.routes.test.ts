...
describe('DELETE /tours/:id', () => {
  ...
  it('returns 409 when an admin deletes a tour which has bookings', async () => {
    let createdUserId: number | undefined;
    let createdTourId: number | undefined;
    ...
    try {
      const { createdUserId: userId } = await createTestUser();
      createdUserId = userId;

      const { createdTourId: tourId } = await createTestTour();
      createdTourId = tourId;

      const { createdBookingId: bookingId } = await createTestBooking(tourId, userId);
      ...
      expect(deleteTourResponse.status).toBe(409);
      expect(deleteTourResponse.body).toHaveProperty('success', false);
      expect(deleteTourResponse.body).toHaveProperty(
        'message',
        'Tour cannot be deleted while it has related records',
      );
      ...
      expect(tour).toStrictEqual({ id: tourId });
      expect(booking).toStrictEqual({ id: bookingId });
    } finally {
      if (createdBookingId !== undefined) {
        const deleteCreatedBookingResponse = await prisma.booking.deleteMany({
          where: { id: createdBookingId },
        });
        expect(deleteCreatedBookingResponse.count).toBe(1);
      }
      if (createdTourId !== undefined) {
        const deleteCreatedTourResponse = await prisma.tour.deleteMany({
          where: { id: createdTourId },
  ...
  it('returns 409 when an admin deletes a tour which has reviews', async () => {
    let createdUserId: number | undefined;
    let createdTourId: number | undefined;
    ...
    try {
      ...
      expect(tour).toStrictEqual({ id: tourId });
      expect(review).toStrictEqual({ id: reviewId });
    } finally {
...
describe('POST /tours/:tourId/bookings', () => {
  it('creates a booking for the authenticated user', async () => {
    ...
    try {
      ...
      if (createdBookingId !== undefined) {
        ...
        expect(deletedBooking.count).toBe(1);
      }
      if (createdTourId !== undefined) {
        const deletedTour = await prisma.tour.deleteMany({
          where: { id: createdTourId },
        });
        expect(deletedTour.count).toBe(1);
      }
      if (createdUserId !== undefined) {
  ...
  it('returns 401 when the request is unauthenticated', async () => {
    ...
    try {
      ...
      const response = await request(app).post(`/tours/${tourId}/bookings`);
  ...
  it('returns 409 when the user has already booked the tour', async () => {
    let createdTourId: number | undefined;
    let createdUserId: number | undefined;
    let createdBookingId: number | undefined;
    ...
    try {
      ...
      expect(response.body).toHaveProperty(
        'message',
        'User must not have already booked this tour',
      );
    } finally {
      if (createdBookingId !== undefined) {
        const deletedBooking = await prisma.booking.deleteMany({
...
describe('DELETE /tours/:tourId/reviews/:reviewId', () => {
  ...
  it('returns 401 when an unauthenticated user deletes a review', async () => {
    let reviewOwnerId: number | undefined;
    let createdTourId: number | undefined;
    let createdReviewId: number | undefined;
    ...
    try {
      ...
      const deleteReviewResponse = await request(app).delete(
        `/tours/${tourId}/reviews/${createdReviewId}`,
      );

      expect(deleteReviewResponse.status).toBe(401);
      expect(deleteReviewResponse.body).toHaveProperty('success', false);
      expect(deleteReviewResponse.body).toHaveProperty('message', 'Authentication required');
      ...
      expect(persistedReview).toStrictEqual({
        id: reviewId,
        ...createReviewPayload,
      });
    } finally {
  ...
  it("returns 403 when a logged-in user deletes another user's review", async () => {
    ...
    try {
      ...
      expect(deleteReviewResponse.status).toBe(403);
      expect(deleteReviewResponse.body).toHaveProperty('success', false);
      expect(deleteReviewResponse.body).toHaveProperty(
        'message',
        'Review belongs to different user',
      );
  ...
  it('returns 404 when deleting an unknown review', async () => {
    let createdUserId: number | undefined;
    let createdTourId: number | undefined;
    ...
    try {
      ...
      const deleteReviewResponse = await request(app)
        .delete(`/tours/${tourId}/reviews/${unknownReviewId}`)
        .set('Authorization', `Bearer ${token}`);

      expect(deleteReviewResponse.status).toBe(404);
      expect(deleteReviewResponse.body).toHaveProperty('success', false);
      expect(deleteReviewResponse.body).toHaveProperty('message', 'Review not found');

      const unknownReview = await prisma.review.findUnique({
        where: { id: unknownReviewId },
      });
  ...
  it('returns 404 when deleting a review that belongs to a different tour', async () => {
    let createdUserId: number | undefined;
    let createdTourAId: number | undefined;
    let createdTourBId: number | undefined;
    let createdReviewId: number | undefined;
    ...
    try {
      ...
      const deleteReviewResponse = await request(app)
        .delete(`/tours/${tourBId}/reviews/${createdReviewId}`)
        .set('Authorization', `Bearer ${token}`);
